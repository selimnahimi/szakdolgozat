...
class InstrumentUtils {
    ...
    static TRACE_STATEMENT = '__crumb__(%s);'
    ...
    static shouldTraceStatement(node) {
        return this.allowedStatementTypes.includes(node.type);
    }
    ...
    static generateFunctionId(funcName, fileName, line) {
        return funcName + '@' + fileName + '::' + line;
    }

    static generateStatementId(node, filename, line) {
        return node.type + '@' + filename + '::' + line;
    }

    static searchFunctionParent(node) {
    ...
    static getFunctionParentId(node, filename, isWrapped) {
        let parentFunction = this.searchFunctionParent(node);
        let parentFunctionName = this.getFunctionName(parentFunction);
        let parentLineStart = isWrapped ? parentFunction.loc.start.line - 1 : parentFunction.loc.start.line;
        return this.generateFunctionId(parentFunctionName, filename, parentLineStart);
    }
    ...
    static getFunctionName(node) {
        ...
        if (node.type === Syntax.FunctionDeclaration) return false;
    ...
    static traceStatement(node, filename, parentFuncId, line, lineEnd, col, colEnd) {
        let statementId = InstrumentUtils.generateStatementId(node, filename, line);

        let object = this.generateStatementTraceObject(node, filename, parentFuncId, line, lineEnd, col, colEnd);
        return util.format(this.TRACE_STATEMENT, object);
    }

    static generateStatementTraceObject(node, filename, parentFunctionId, line, lineEnd, col, colEnd) {
        let statementId = InstrumentUtils.generateStatementId(node, filename, line);

        return {
            funcId: parentFunctionId,
